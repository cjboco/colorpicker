...
(function ($) {
	...
	$.widget("vanderlee.colorpicker", {
		options: {
			...
			limit: '',
			// Limit color "resolution": '', 'websafe', 'nibble', 'binary'
			mode: 'h',
			...
			if (this.element[0].nodeName.toLowerCase() === 'input') {
				...
				$(document).mousedown(function (event) {
					...
					$.each(parents, function (idx, p) {
						if (self.button !== null && p === self.button[0]) {
							return;
						}
					});
		...
		_generate: function () {
			var self = this,
				part, parts_list, layout_parts = [];

			// Determine the parts to include in this colorpicker
			if (typeof self.options.parts === 'string') {
				if (_parts_lists.hasOwnProperty(self.options.parts)) {
			...
			self.parts = {};
			$.each(parts_list, function (index) {
			...
			if (!self.generated) {
				...
				$.each(self.options.layout, function (index) {
					layout_parts.push({
						part: index,
		...
		_layoutTable: function (layout) {
			var bitmap, x, y, width, height, index, cell, html, w, h, classes = [],
				colspan;
			...
			for (y = 0; y < height; ++y) {
				...
				do {
					if (cell !== undefined && x == cell.pos[0] && y == cell.pos[1]) {
						...
						html += '<td id="ui-colorpicker-' + cell.part + '-container"' + (cell.pos[2] > 1 ? ' colspan="' + cell.pos[2] + '"' : '') + (cell.pos[3] > 1 ? ' rowspan="' + cell.pos[3] + '"' : '') + (classes.length > 0 ? ' class="' + classes.join(' ') + '"' : '') + ' valign="top"></td>';
		...
		_effectGeneric: function (show, slide, fade, callback) {
			...
			if ($.effects && $.effects[self.options.showAnim]) {
				...
				self.dialog[(self.options.showAnim === 'slideDown' ? slide : (self.options.showAnim === 'fadeIn' ? fade : show))]((self.options.showAnim ? self.options.duration : null), callback);
		...
		_parts: {
			header: function (inst) {
				...
				_html = function () {
					return '<div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">' + '<span class="ui-dialog-title">' + inst.options.title + '</span>' + '<a href="#" class="ui-dialog-titlebar-close ui-corner-all" role="button">' + '<span class="ui-icon ui-icon-closethick">close</span></a></div>';
				};
			...
			map: function (inst) {
				...
				_html = function () {
					var html = '<div id="ui-colorpicker-map" class="ui-colorpicker-border">' + '<span id="ui-colorpicker-map-layer-1">&nbsp;</span>' + '<span id="ui-colorpicker-map-layer-2">&nbsp;</span>' + (inst.options.alpha ? '<span id="ui-colorpicker-map-layer-alpha">&nbsp;</span>' : '') + '<span id="ui-colorpicker-map-layer-pointer"><span id="ui-colorpicker-map-pointer"></span></span></div>';
					return html;
				};
			...
			bar: function (inst) {
				...
				_html = function () {
					var html = '<div id="ui-colorpicker-bar" class="ui-colorpicker-border">' + '<span id="ui-colorpicker-bar-layer-1">&nbsp;</span>' + '<span id="ui-colorpicker-bar-layer-2">&nbsp;</span>' + '<span id="ui-colorpicker-bar-layer-3">&nbsp;</span>' + '<span id="ui-colorpicker-bar-layer-4">&nbsp;</span>';

					if (inst.options.alpha) {
						html += '<span id="ui-colorpicker-bar-layer-alpha">&nbsp;</span>' + '<span id="ui-colorpicker-bar-layer-alphabar">&nbsp;</span>';
					}
			...
			inputs: function (inst) {
				...
				_getColorFromInputs = function () {
					...
					$('#ui-colorpicker-r .ui-colorpicker-number', e).val() / 255, $('#ui-colorpicker-g .ui-colorpicker-number', e).val() / 255, $('#ui-colorpicker-b .ui-colorpicker-number', e).val() / 255, $('#ui-colorpicker-a .ui-colorpicker-number', e).val() / 100, $('#ui-colorpicker-h .ui-colorpicker-number', e).val() / 360, $('#ui-colorpicker-s .ui-colorpicker-number', e).val() / 100, $('#ui-colorpicker-v .ui-colorpicker-number', e).val() / 100);
				};
				...
				this.repaint = function () {
					...
					$.each(c, function (index, value) {
						...
						if (!$('#ui-colorpicker-' + index + ' .ui-colorpicker-number', e).is(':focus') && $('#ui-colorpicker-' + index + ' .ui-colorpicker-number', e).val() !== v) {
			...
			preview: function (inst) {
				...
				_html = function () {
					return '<div id="ui-colorpicker-preview">' + '<span class="ui-colorpicker-border">' + '<div id="ui-colorpicker-preview-initial"><div id="ui-colorpicker-preview-initial-alpha"></div></div>' + '<div id="ui-colorpicker-preview-current"><div id="ui-colorpicker-preview-current-alpha"></div></div>' + '</span>' + '</div>';
				};
			...
			footer: function (inst) {
				...
				_html = function () {
					return '<div class="ui-dialog-buttonpane ui-widget-content">' + (inst.options.alpha ? '<button class="ui-colorpicker-transparent">transparent</button>' : '') + (inst.inline ? '' : '<div class="ui-dialog-buttonset">' + '<button class="ui-colorpicker-close">Done</button>' + '</div>') + '</div>';
				};
		...
		_parseColor: function (color) {
			...
			if (m) {
				return [
				m[1] / 100, m[2] / 100, m[3] / 100, m[4] / 100];
			}
			...
			if (m) {
				return [
				parseInt(m[1] + m[1], 16) / 255, parseInt(m[2] + m[2], 16) / 255, parseInt(m[3] + m[3], 16) / 255];
			}
		...
		Color: function () {
			...
			this.equals = function (rgb) {
				return rgb[0] === this.r && rgb[1] === this.g && rgb[2] === this.b;
			};
